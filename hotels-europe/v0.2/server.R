...
shinyServer(function(input, output, session) {
    ...
    data_reg <- data %>% mutate(ln_price = log(price),
        ...
        select(hotel_id, date, price, ln_price, N_rating_review, ln_N_rating_review, distance, ln_distance, distance_alter, Offer_Cat, stars, Type, city_actual, District, city,rating, city_actual) %>% 
    ...
    data_re <- reactive({
      if(input$tabs == 'down'){
        loading(input$down_sel_city)
        data_ <- data %>% filter(city == input$down_sel_city,
                                 date == input$down_sel_date)
      }
      else if(input$tabs == 'desc'){
      ...
      data_ <- data_ %>%
        select(setdiff(names(data_), c("month", "year",  "weekend",
    ...
    reactive_data <- reactive({
        ...
        data_ <- data %>%
            select(setdiff(names(data), c("month", "year", "",  "weekend",
        ...
        for(i in names_to_filter){
            if(class(data[[i]]) != 'factor'){
                ...
                data_ <- data_ %>%
                  dplyr::filter(get(i) >= filter_obj[1] & get(i) <= filter_obj[2])
            }
            else{
                filter_obj <- eval(parse(text = paste0("input$filter_", i)))
                data_ <- data_ %>%
                  dplyr::filter(get(i) %in% filter_obj)
            }
        }
    ...
    reactive_data_reg <- reactive({
        data_ <- data_reg %>%
            select(setdiff(names(data_reg), c("month", "year", "",  "weekend",
                                          "holiday", "country", "offer", "ratingta", "ratingta_count")))
    ...
    ### Pick a city
    output$down_sel_city <- renderUI({
      selectInput(inputId = "down_sel_city",
                  label = "Select a city:",
    ...
    ### Pick a date
    output$down_sel_date <- renderUI({
      selectInput(inputId = "down_sel_date",
                  label = "Select a date:",
    ...
    newdata <- reactive({
      data.frame(reactive_data())
    })
    
    filtered_data <- reactive({
      library(dplyr)
      filter(newdata(), city %in% input$down_sel_city & date %in% input$down_sel_date)
    })
    ...
    pressbutton <- eventReactive(input$mybutton, {
      input$n
    })
    ...
    output$down_nrows <- renderUI({
      loading(input$down_sel_city)
      loading(input$down_sel_date)
      
      data_ <- reactive_data()
      
      data_ <- data_ %>% 
        dplyr::filter(city %in% input$down_sel_city & date %in% input$down_sel_date) %>% 
        drop_na()
    ...
    output$downloadData <- downloadHandler(
      filename = function() {
        paste0("Hotels_in_",input$down_sel_city, "_on_", input$down_sel_date, "_Downloaded_on_", Sys.Date(), ".", input$filetype)
      },
      ...
      content = function(file) {
        ...
        write.table(filtered_data(), file, sep = sep,
                    row.names = FALSE)
      }
    )
...
output$tab_description <- renderUI({
  ...
  if(input$tabs == 'down'){
    text <- '<p dir="ltr" style="line-height:1.38;margin-top:0pt;margin-bottom:0pt;"><h3> Tab description: </h3><span style="font-size:11pt;font-family:Arial;color:#000000;background-color:transparent;font-weight:700;font-style:normal;font-variant:normal;text-decoration:none;vertical-align:baseline;white-space:pre;white-space:pre-wrap;"> Download data </span></p>
         <p dir="ltr" style="line-height:1.38;margin-top:0pt;margin-bottom:0pt;"><span style="font-size:11pt;font-family:Arial;color:#000000;background-color:transparent;font-weight:400;font-style:normal;font-variant:normal;text-decoration:none;vertical-align:baseline;white-space:pre;white-space:pre-wrap;">We show the dataset for a single city and date. Applying the given filters, the dataset can be exported in multiple file types.&nbsp;</span></p>'
  }
